...
async function runHeatAgent({ lat, lng, polygonAoi }) {
  let { data, source, confidence } = await fortyguard.createHeatmap({ lat, lng, polygonAoi, thresholdC: THRESHOLD_C });
  ...
  if (!validation.ok && source.startsWith("live")) {
    const retry = await fortyguard.createHeatmap({ lat, lng, polygonAoi, thresholdC: THRESHOLD_C });
    data = retry.data;
    source = retry.source;
